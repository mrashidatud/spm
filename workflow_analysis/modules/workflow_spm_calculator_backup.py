...
def is_valid_storage_match(prod_storage, cons_storage, prod_task_name=None, cons_task_name=None):
    ...
    if '-' not in prod_storage:
        if prod_storage == "beegfs" and cons_storage in ("beegfs-ssd", "beegfs-tmpfs", "beegfs"):
            return True
...
def handle_stage_in_none_producers(prod_task_name, producer_group, consumer_df, WFG, edge_count, processed_pairs, processed_pairs_debug, debug=False):
    ...
    for _, producer_row in producer_group.iterrows():
        ...
        for _, consumer_row in consumer_subset.iterrows():
            ...
            # Removed repetitive debug output for storage types
            ...
            for prod_storage in all_prod_storages:
                for cons_storage in all_cons_storages:
                    ...
                    for prod_key in prod_keys:
                        ...
                        # Debug output only when transfer rates are being calculated
                        if debug and prod_task_name.startswith('stage_in-') and estT_prod > 0:
                            print(f"[DEBUG] Transfer rate calculation: {prod_task_name} -> {consumer_row['taskName']}")
                            print(f"  Producer: {prod_storage}_{n_prod} (estT_prod: {estT_prod:.4f})")
                        ...
                        for cons_key in cons_keys:
                            ...
                            # Removed repetitive debug output for consumer side
                            ...
                            # Debug output showing estT values for each task
                            if debug and prod_task_name.startswith('stage_in-'):
                                print(f"[DEBUG] Task timing: {prod_task_name} -> {consumer_row['taskName']}")
                                ...
                                print(f"  estT_prod: {estT_prod:.4f}s, estT_cons: {estT_cons:.4f}s")
                                print(f"prod_op: {prod_op}, cons_op: {cons_op}")
                                print("  ---")
...
def add_producer_consumer_edge(WFG, prod_nodes, cons_nodes, debug=False, workflow_name=None):
    ...
    # Removed repetitive debug output for missing pairs
    ...
    for prod_task_name, producer_group in producer_groups:
        ...
        if prod_task_name.startswith('stage_in-'):
            # Debug: Check what operations are in this producer group
            if debug:
                operations_in_group = [convert_operation_to_string(prod.get('operation')) for _, prod in producer_group.iterrows()]
                print(f"[DEBUG] stage_in task '{prod_task_name}' has operations: {operations_in_group}")
                has_cp_scp = any(op in ['cp', 'scp'] for op in operations_in_group)
                print(f"[DEBUG] Has cp/scp operations: {has_cp_scp}")
            
            if any(convert_operation_to_string(prod.get('operation')) in ['cp', 'scp'] for _, prod in producer_group.iterrows()):
                # Extract the actual task name from stage_in-{taskName}
                actual_task_name = prod_task_name.replace('stage_in-', '')
                            ...
                            for cons_storage in all_cons_storages:
                                ...
                                for prod_key in prod_keys:
                                    ...
                                    for cons_key in cons_keys:
                                        ...
                                        # Debug output for cp/scp operations
                                        if debug:
                                            print(f"[DEBUG] CP/SCP timing: {prod_task_name} -> {consumer_row['taskName']}")
                                            print(f"  Producer: {prod_storage}_{n_prod} (estT_prod: {estT_prod:.4f}s)")
                                            print(f"  Consumer: {cons_storage}_{n_cons} (estT_cons: {estT_cons:.4f}s)")
                                            print(f"[DEBUG] Task timing: {prod_task_name} -> {consumer_row['taskName']}")
                                            print(f"  prod_aggregateFilesizeMB: {prod_aggregateFilesizeMB}")
                                            print(f"  cons_aggregateFilesizeMB: {cons_aggregateFilesizeMB}")
                                            print(f"  prod_estimated_trMiB: {prod_estimated_trMiB}")
